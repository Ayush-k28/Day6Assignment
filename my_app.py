...
if add_selectbox == "About":
    ...
    colour = st.sidebar.radio("Choose the background colour for your image",
     ('green', "red",'goku background',"starry night sky background"))
    count=0 #initializing a count variable for differenting between image and colour backgrounds
    if colour == "green":
        BG_COLOR = (0,255, 0)
    elif colour == "goku background":
        BG_COLOR = cv2.imread('goku.jpg')
        count=1
        
    elif colour == "red":
        BG_COLOR = (255,0,0)
    elif colour == "starry night sky background":
        BG_COLOR = cv2.imread('starry.jpg') 
        count=1         
    ...
    with mp_selfie_segmentation.SelfieSegmentation(
        model_selection=0) as selfie_segmentation:
            ...
            if image_file_path is not None:
                ...
                if count==1:
                    bg_image = cv2.resize(BG_COLOR, (image.shape[1], image.shape[0]))
